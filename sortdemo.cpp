...
wxString csCaption;
  ...
  m_pMainWnd = (wxWindow *)
    new CMainFrame(m_pDocManager, (wxFrame *) NULL, -1, wxString(_T("Sort Demo")),
                   wxPoint(0, 0), wxSize(800, 640), wxDEFAULT_FRAME_STYLE);
...
void CAboutDlg::Update(BOOL bUpdateFlag)
{
  ...
  else            // data to controls
  {
    ...
    // The copyright refers primarily to the wxWidgets version, which was written in 2005
    // However, nearly all of the source files were originally written for MFC in 1995
    // and then adapted for wxWidgets.  So the source file comments include a copyright
    // with a somewhat wider date range.  This is not inconsistent.
    SetDlgItemText(lAboutBoxID1, __T("Copyright (c) 2005-2020 by 'Bombastic Bob' Frazier"));
  }
}
...
void CAboutDlg::OnInitDialog(wxInitDialogEvent &evt)
{
  ...
  Update(0);
  Centre(); // I like the way it uses British spellings in wxWidgets
}
...
UINT ui1;
   ...
   // TODO:  consider using 'random' instead
   srand(MyGetTickCount());

   for(ui1=0; ui1 < MyGetTickCount() % 17; ui1++)
   {
     rand(); // to help randomize better
   }
...
//  return((unsigned long)tb.time * 1000L + (unsigned long)tb.millitm);
  return (unsigned long)(MyGetMicros() / 1000);
}
...
{
#ifdef WIN32
  return (UINT64)GetTickCount * (UINT64)1000;
#else // !WIN32
struct timeval tv;
...
extern "C" void MySleep(unsigned long lMicroSeconds)
{
  ...
  Sleep(lMicroSeconds / 1000);
#else // WIN32
  wxThread *pT = wxThread::This();
  if(pT)
  {
    unsigned long long lStartTick = MyGetMicros();
    ...
    do
    {
      ...
      lTemp = (unsigned long)(MyGetMicros() - lStartTick);
      if(lTemp > 100)
        lTemp = 100;

      usleep(lTemp);

    } while((MyGetMicros() - lStartTick) < (unsigned long long)lMicroSeconds);
  }
  else
  {
    wxWakeUpIdle();  // force idle processing to 'wake up'

    usleep(lMicroSeconds);
  }
#endif // WIN32
