...
//   pDC->BeginDrawing(); this doesn't do anything anyway and is now deprecated
   ...
   if(rct.height < 2 * N_DIMENSIONS(pData) + 4)
   {
     m_iLineHeight = 1;
   }
   else
   {
     m_iLineHeight = (rct.height - 4) / N_DIMENSIONS(pData);
   }
...
//   pDC2->BeginDrawing(); this doesn't do anything anyway and is now deprecated
   ...
   for(iIndex=0; iIndex < m_iLineHeight; iIndex++)
   {
     pDC2->SetPen(cpen);
     pDC2->DrawLine(rct.x + 2,
                    rct.y + 2 + uiIndex * m_iLineHeight + iIndex,
                    rct.x + 2 + pData[uiIndex] * m_iLineWidth,
                    rct.y + 2 + uiIndex * m_iLineHeight + iIndex);

     pDC2->SetPen(cpenWhite);
     pDC2->DrawLine(rct.x + 2 + pData[uiIndex] * m_iLineWidth,
                    rct.y + 2 + uiIndex * m_iLineHeight + iIndex,
                    rct.x + rct.width - 2,
                    rct.y + 2 + uiIndex * m_iLineHeight + iIndex);
   }
