...
extern "C" const int IDM_SORT_SPEED = XRCID("IDM_SORT_SPEED");
...
BEGIN_EVENT_TABLE(CMainFrame, wxDocParentFrame)
  ...
  EVT_MENU(IDM_SORT_SPEED, CMainFrame::OnSortSpeed)
  EVT_UPDATE_UI(IDM_SORT_SPEED, CMainFrame::OnUpdateSortSpeed)
...
void SortInfo(const wxChar *szSortName)
{
   wxSprintf(szSortInfo, __T("Sort: %s\n%d swaps, %d compares,\n%d milliseconds"),
             ...
             (int)(MyGetTickCount() - dwStartTime));
...
void SortInfoT(const wxChar *szSortName)
{
   wxSprintf(szSortInfo, __T("Sort: %s\n%d swaps, %d compares,\n%d sched, %d milliseconds\nMax Threads: %d"),
             szSortName, iSwaps, iCompares,
             iSchedWork, (int)(MyGetTickCount() - dwStartTime),
             iThreadCountMax);
...
void CMainFrame::OnSortSpeed(wxCommandEvent &evt)
{
//   if(!theApp.m_pSortThread)
//     CreateSortThread(ID_SORT_BUBBLE);

   // for now just switch between these two

   if(iTimeDelay <= SORT_FAST_SPEED * 2)
   {
     GetMenuBar()->Check(IDM_SORT_SPEED, 0);
     iTimeDelay = SORT_SLOW_SPEED;
   }
   else
   {
     GetMenuBar()->Check(IDM_SORT_SPEED, 1);
     iTimeDelay = SORT_FAST_SPEED;
   }
...
void CMainFrame::OnUpdateSortSpeed(wxUpdateUIEvent &cmdUI)
{
   cmdUI.Enable(1);

   if(iTimeDelay <= 2 * SORT_FAST_SPEED)
     cmdUI.Check(1);
   else
     cmdUI.Check(0);
